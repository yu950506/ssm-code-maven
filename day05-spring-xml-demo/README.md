# Spring 基于xml配置文件的方式配置案例
## 1. 项目介绍
* 3层架构，采用Spring降低各模块之间的耦合      
>dao:与数据库交互，提供两种实现:c3p0+DBUtils ,Spring jdbc + Druid             
  service 调用dao层    
  controller 调用service  

* Spring 的配置文件，这里通过容器创建对象
```xml
...
## 2. 项目总结
 通过这个案例熟悉了Spring容器管理对象的方式
 和依赖注入对象的值，还深刻体会到了程序间的解耦